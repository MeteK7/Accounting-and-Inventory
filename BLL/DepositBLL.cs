...
namespace BLL
{
    public class DepositBLL
    {
        ...
        public int GetLastDepositId()
        {
            ...
            {
                depositId = Convert.ToInt32(dataTable.Rows[initialIndex]["id"]);//We defined this code out of the for loop below because all of the products has the same invoice number in every sale. So, no need to call this method for every products again and again.
            }
            else//If there is no any invoice number, that means it is the first sale. So, assing invoiceNo with 0;
            {
                depositId = initialIndex;
            }
            return depositId;
        }
    }
